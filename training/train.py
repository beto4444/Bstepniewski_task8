...
class DataProcessor:
    ...
    def prepare_data(self, max_rows: int = None) -> (pd.DataFrame, pd.DataFrame):
        logging.info("Preparing data for training...")
        df = pd.read_csv(os.path.join(DATA_DIR, conf["train"]["table_name"]))
        x = df.iloc[:, :-1]
        y = df.iloc[:, -1]
        x, y = self.data_rand_sampling(x, y, max_rows)
        return x, y

    def data_rand_sampling(
        self, x: pd.DataFrame, y: pd.DataFrame, max_rows: int
